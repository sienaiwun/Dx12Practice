...
void createQuad(float4 v0, float4 v1, float4 v2, float4 v3, float4 uni_color,float3 normal, inout TriangleStream< GSOutput > output)
{
    GSOutput element;
    element.pos = v0;
...
)
{
    ...
    if (colors[0].a >= EPSILON)
    {
        ...
        createQuad(v0, v2, v6, v4,  colors[0], normal, output);
    }

    if (colors[1].a >= EPSILON)
    {//right
        float3 normal = float3(1.0f, 0.0f, 0.0f);
        createQuad(v1, v5, v7, v3, colors[1], normal, output);
    }
    ...
    if (colors[2].a >= EPSILON)
    {
        ...
        createQuad(v0, v4, v5, v1, colors[2], normal,output);
    }
    ...
    if (colors[3].a >= EPSILON)
    {
        // back
        float3 normal = float3(0.0f, 0.0f, 1.0f);
        createQuad(v2, v3, v7, v6, colors[3], normal, output);
    }
    ...
    if (colors[4].a >= EPSILON)
    {
        ...
        createQuad(v0, v1, v3, v2, colors[4], normal, output);
    }
    ...
    {
        float3 normal = float3(0.0f, 1.0f, 0.0f);
        createQuad(v4, v6, v7, v5, colors[5], normal, output);
    }
